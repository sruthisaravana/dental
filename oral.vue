...
const mode = ref('create'); // Changed from 'view' to 'create' as default
...
const createNewExaminationObject = () => ({
  ...
  mobility_scores: {}, // Format: { "11": "1", "12": "0" }
  calculus_stains: {}, // Format: { "11": "+", "12": "++" }
...
// --- STATIC DATA ---
// Predefined conditions with cartoon tooth images (matching RecordDetailView)
...
const mobilityGrades = ['0', '1', '2', '3'];
const calculusLevels = ['+', '++', '+++'];

// --- COMPUTED PROPERTIES ---
const conditionsForView = computed(() => {
    if (!selectedExamination.value || !selectedExamination.value.condition_of_teeth) return {};
    return selectedExamination.value.condition_of_teeth;
});
...
const getToothBorderClass = (condition) => {
  const conditionColors = {
    'Healthy': 'border-emerald-400',
    'Decayed': 'border-rose-400',
    'Filled': 'border-sky-400',
    'Missing': 'border-slate-400',
    'Cracked': 'border-orange-400',
    'Wisdom': 'border-violet-400',
    'Fully Decayed': 'border-rose-600',
    'Root Canal': 'border-violet-500',
    'Root Canal Done': 'border-violet-600',
    'Impacted': 'border-amber-500',
    'Tooth Removed': 'border-slate-600',
    'Removed': 'border-slate-600',
    'Stained': 'border-yellow-400',
    'Worn': 'border-orange-300',
    'Abrasion': 'border-orange-500',
    'Erosion': 'border-rose-300',
    'Chipped': 'border-amber-400',
    'Fractured Cusp': 'border-orange-600',
    'Hypersensitive': 'border-pink-400',
    'Periapical Abscess': 'border-rose-700',
    'Gingivitis': 'border-pink-300',
    'Periodontitis': 'border-rose-500',
    'Pericoronitis': 'border-pink-500',
    'Supernumerary': 'border-indigo-300',
    'Un-erupted Primary': 'border-slate-300',
    'Retained Primary': 'border-slate-400',
    'Malocclusion': 'border-indigo-500',
    'Attrition': 'border-amber-600',
    'Enamel Hypoplasia': 'border-yellow-500',
    'Amelogenesis Imperfecta': 'border-yellow-600',
    'Dentinogenesis Imperfecta': 'border-blue-400',
    'Gingival Recession': 'border-pink-600'
  };
  return conditionColors[condition] || 'border-gray-300';
};
...
const getToothCurrentCondition = (toothNumber) => {
  ...
  // Fall back to examination data in view mode
  if (mode.value === 'view') {
    return conditionsForView.value[toothNumber] || null;
  } else {
    // In edit mode, use current form data
...
const switchToCreateMode = () => {
    Object.assign(newExaminationData, createNewExaminationObject()); // Reset form
    activeTab.value = 'adult'; // Reset to adult tab
    selectedCondition.value = 'Healthy'; // Reset to default
    otherConditionText.value = ''; // Clear other condition text
    newCustomCondition.value = ''; // Clear custom condition input
    showFloatingNotification.value = true; // Show notification again
    mode.value = 'create';
    // Reset search state
    searchQuery.value = '';
...
const formatPayload = (data) => {
  // Clean up mobility scores and calculus stains - remove undefined values
  const cleanMobilityScores = Object.fromEntries(
    Object.entries(data.mobility_scores).filter(([, value]) => value !== undefined && value !== null)
  );
  
  const cleanCalculusStains = Object.fromEntries(
    Object.entries(data.calculus_stains).filter(([, value]) => value !== undefined && value !== null)
  );
  ...
  const payload = {
    ...
    mobility_scores: cleanMobilityScores,
    calculus_stains: cleanCalculusStains,
    ...
    <div class="relative h-full w-full">
      <div class="bg-white dark:bg-gray-900 w-full h-full flex flex-col">
        ...
        <header class="flex-shrink-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4">
          <div class="flex justify-between items-center">
            ...
            <div class="flex items-center gap-4">
              <div class="flex items-center space-x-1 p-1 bg-gray-200 dark:bg-gray-700 rounded-lg">
                <button @click="mode = 'view'" :class="['px-3 py-1 text-sm font-semibold rounded-md', mode === 'view' ? 'bg-white dark:bg-blue-600 text-blue-600 dark:text-white shadow' : 'text-gray-600 dark:text-gray-300']">
                  View Records
                </button>
                <button @click="switchToCreateMode" :class="['px-3 py-1 text-sm font-semibold rounded-md', mode === 'create' ? 'bg-white dark:bg-blue-600 text-blue-600 dark:text-white shadow' : 'text-gray-600 dark:text-gray-300']">
                  New Examination
                </button>
              </div>
        ...
        <main class="flex-grow overflow-hidden">
          ...
          <!-- VIEW MODE -->
          <div v-else-if="mode === 'view'" class="h-full overflow-y-auto p-6">
            <div v-if="examinations.length === 0" class="text-center py-24">
              <Smile class="h-16 w-16 mx-auto mb-4 text-gray-300" />
              <p class="text-lg font-medium text-gray-600 dark:text-gray-400">No oral examinations found</p>
              <button @click="switchToCreateMode" class="mt-4 inline-flex items-center px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 shadow">
                <PlusCircle class="h-4 w-4 mr-2" />Create First Examination
              </button>
            </div>
            <div v-else>
              <div class="mb-6">
                <div class="flex justify-between items-center mb-4">
                  <h3 class="text-lg font-semibold">
                    ...
                    <span class="text-base font-normal text-gray-600 dark:text-gray-400 ml-2">
                      ({{ examinations.length }} examination{{ examinations.length !== 1 ? 's' : '' }} found)
                    </span>
                  </h3>
              <div class="flex items-center gap-3">
                <select v-model="selectedExamination" class="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-xl px-4 py-3 text-sm font-medium shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 min-w-[200px]">
                  <option v-for="exam in examinations" :key="exam.id" :value="exam">
                    📅 {{ formatDate(exam.exam_date) }}
                  </option>
                </select>
                <button @click="switchToCreateMode" class="inline-flex items-center px-4 py-3 bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-medium rounded-xl hover:from-emerald-600 hover:to-teal-700 shadow-sm transition-all duration-200">
                ...
                <div v-if="selectedExamination && selectedExamination.patient" class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 mb-4">
                  <div class="flex items-center">
                    <div class="flex-shrink-0">
                      <div class="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center">
                        <span class="text-white font-medium">{{ selectedExamination.patient.first_name.charAt(0) }}{{ selectedExamination.patient.last_name.charAt(0) }}</span>
                      </div>
                    </div>
                    <div class="ml-3">
                      <p class="text-sm font-medium text-blue-900 dark:text-blue-100">
                        {{ selectedExamination.patient.first_name }} {{ selectedExamination.patient.last_name }}
                      </p>
                      <p class="text-xs text-blue-700 dark:text-blue-300">
                        Examination on {{ formatDate(selectedExamination.exam_date) }}
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div v-if="selectedExamination" class="space-y-6">
                <!-- Read-only Dental Chart -->
                <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow">
                  <h4 class="font-semibold text-center mb-4">Condition of Teeth on {{ formatDate(selectedExamination.exam_date) }}</h4>
                  
                  <!-- Condition Legend -->
                  <div class="mb-6 bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                    <h5 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Condition Legend</h5>
                    <div class="flex flex-wrap gap-x-4 gap-y-2 text-xs">
                      <!-- Primary Conditions -->
                      <div class="flex items-center">
                        <span class="h-3 w-3 bg-emerald-400 rounded mr-2"></span>
                        <span class="text-gray-700 dark:text-gray-300">Healthy</span>
                      </div>
                      <div class="flex items-center">
                        <span class="h-3 w-3 bg-rose-400 rounded mr-2"></span>
                        <span class="text-gray-700 dark:text-gray-300">Decayed</span>
                      </div>
                      <div class="flex items-center">
                        <span class="h-3 w-3 bg-sky-400 rounded mr-2"></span>
                        <span class="text-gray-700 dark:text-gray-300">Filled</span>
                      </div>
                      <div class="flex items-center">
                        <span class="h-3 w-3 bg-slate-400 rounded mr-2"></span>
                        <span class="text-gray-700 dark:text-gray-300">Missing</span>
                      </div>
                      <div class="flex items-center">
                        <span class="h-3 w-3 bg-orange-400 rounded mr-2"></span>
                        <span class="text-gray-700 dark:text-gray-300">Cracked</span>
                      </div>
                      <div class="flex items-center">
                        <span class="h-3 w-3 bg-violet-400 rounded mr-2"></span>
                        <span class="text-gray-700 dark:text-gray-300">Wisdom</span>
                      </div>
                      <!-- Additional Conditions -->
                      <div class="flex items-center">
                        <span class="h-3 w-3 bg-yellow-400 rounded mr-2"></span>
                        <span class="text-gray-700 dark:text-gray-300">Stained</span>
                      </div>
                      <div class="flex items-center">
                        <span class="h-3 w-3 bg-pink-300 rounded mr-2"></span>
                        <span class="text-gray-700 dark:text-gray-300">Gingivitis</span>
                      </div>
                      <div class="flex items-center">
                        <span class="h-3 w-3 bg-rose-500 rounded mr-2"></span>
                        <span class="text-gray-700 dark:text-gray-300">Periodontitis</span>
                      </div>
                      <div class="flex items-center">
                        <span class="h-3 w-3 bg-pink-400 rounded mr-2"></span>
                        <span class="text-gray-700 dark:text-gray-300">Hypersensitive</span>
                      </div>
                      <div class="flex items-center">
                        <span class="h-3 w-3 bg-rose-700 rounded mr-2"></span>
                        <span class="text-gray-700 dark:text-gray-300">Abscess</span>
                      </div>
                      <div class="flex items-center">
                        <span class="h-3 w-3 bg-indigo-500 rounded mr-2"></span>
                        <span class="text-gray-700 dark:text-gray-300">Malocclusion</span>
                      </div>
                    </div>
                  </div>
                      ...
                      <div v-for="tooth in [...adultPermanent.upperRight, ...adultPermanent.upperLeft]" :key="tooth" class="text-center">
                        <button 
                          ...
                          <div class="relative">
                            ...
                            <div :class="['absolute inset-0 rounded-lg opacity-60 transition-opacity duration-200', getToothConditionBackgroundClass(tooth)]"></div>
                            <div class="w-10 h-14 mx-auto flex items-center justify-center">
                              <img v-if="toothImageExists[tooth]" :src="getToothImagePath(tooth)" @error="imageError(tooth)" class="w-full h-full object-contain relative z-10 transition-all duration-200 group-hover:brightness-110" />
                              <div v-else class="w-8 h-12 bg-gray-100 dark:bg-gray-600 rounded-md border-2 relative z-10">🦷</div>
                            </div>
                          </div>
                          <div class="mt-1">
                            <span class="block text-xs font-semibold transition-colors duration-200 group-hover:text-blue-600 dark:group-hover:text-blue-400">{{ tooth }}</span>
                            <div v-if="getToothCurrentCondition(tooth)" class="text-xs font-medium px-1 py-0.5 rounded mt-1" :class="getConditionDisplayClass(getToothCurrentCondition(tooth))">
                    ...
                    <div class="grid grid-cols-16 gap-1 justify-items-center max-w-6xl mx-auto">
                      <div v-for="tooth in [...adultPermanent.lowerRight.slice().reverse(), ...adultPermanent.lowerLeft.slice().reverse()]" :key="tooth" class="text-center">
                        <button 
                          ...
                          <div class="mb-1">
                            <span class="block text-xs font-semibold transition-colors duration-200 group-hover:text-blue-600 dark:group-hover:text-blue-400">{{ tooth }}</span>
                            <div v-if="getToothCurrentCondition(tooth)" class="text-xs font-medium px-1 py-0.5 rounded mb-1" :class="getConditionDisplayClass(getToothCurrentCondition(tooth))">
                              {{ getConditionDisplayLabel(getToothCurrentCondition(tooth)) }}
                            </div>
                          </div>
                          <div class="relative">
                ...
                <div class="max-w-6xl mx-auto">
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow border border-gray-200 dark:border-gray-700">
                      <h5 class="font-semibold mb-3 flex items-center text-gray-800 dark:text-gray-200">
                        <Calendar class="h-4 w-4 mr-2 text-blue-600"/>
                        Examination Date
                      </h5>
                      <p class="text-gray-600 dark:text-gray-400 text-sm">{{ formatDate(selectedExamination.exam_date) }}</p>
                    </div>

                    <!-- Malocclusion -->
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow border border-gray-200 dark:border-gray-700">
                      <h5 class="font-semibold mb-3 flex items-center text-gray-800 dark:text-gray-200">
                        <Activity class="h-4 w-4 mr-2 text-red-600"/>
                        Malocclusion
                      </h5>
                      <div v-if="selectedExamination.malocclusion && Object.values(selectedExamination.malocclusion).some(v => v)" class="space-y-2">
                        <div v-for="(value, key) in selectedExamination.malocclusion" :key="key" v-show="value" class="text-sm">
                          <span class="inline-block w-2 h-2 bg-red-500 rounded-full mr-2"></span>
                          <span class="text-gray-700 dark:text-gray-300">{{ key.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase()) }}</span>
                        </div>
                      </div>
                      <p v-else class="text-gray-500 text-sm">No malocclusion noted</p>
                    </div>

                    <!-- Extra-Oral Findings -->
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow border border-gray-200 dark:border-gray-700">
                      <h5 class="font-semibold mb-3 flex items-center text-gray-800 dark:text-gray-200">
                        <FileText class="h-4 w-4 mr-2 text-green-600"/>
                        Extra-Oral Findings
                      </h5>
                      <p v-if="selectedExamination.extra_oral_findings" class="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">
                        {{ selectedExamination.extra_oral_findings }}
                      </p>
                      <p v-else class="text-gray-500 text-sm">No findings noted</p>
                    </div>

                    <!-- Mobility Summary -->
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow border border-gray-200 dark:border-gray-700">
                      <h5 class="font-semibold mb-3 flex items-center text-gray-800 dark:text-gray-200">
                        <Zap class="h-4 w-4 mr-2 text-yellow-600"/>
                        Mobility Issues
                      </h5>
                      <div v-if="selectedExamination.mobility_scores && Object.keys(selectedExamination.mobility_scores).length > 0" class="space-y-2">
                        <div v-for="(grade, tooth) in selectedExamination.mobility_scores" :key="`mob-view-${tooth}`" v-show="grade && grade !== '0'" class="text-sm">
                          <span class="font-medium text-gray-700 dark:text-gray-300">Tooth #{{ tooth }}:</span> 
                          <span class="ml-1 px-2 py-1 bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 rounded text-xs">Grade {{ grade }}</span>
                        </div>
                      </div>
                      <p v-else class="text-gray-500 text-sm">No mobility issues noted</p>
                    </div>

                    <!-- Calculus Summary -->
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow border border-gray-200 dark:border-gray-700">
                      <h5 class="font-semibold mb-3 flex items-center text-gray-800 dark:text-gray-200">
                        <Heart class="h-4 w-4 mr-2 text-pink-600"/>
                        Calculus/Stains
                      </h5>
                      <div v-if="selectedExamination.calculus_stains && Object.keys(selectedExamination.calculus_stains).length > 0" class="space-y-2">
                        <div v-for="(level, tooth) in selectedExamination.calculus_stains" :key="`calc-view-${tooth}`" v-show="level" class="text-sm">
                          <span class="font-medium text-gray-700 dark:text-gray-300">Tooth #{{ tooth }}:</span> 
                          <span class="ml-1 px-2 py-1 bg-pink-100 dark:bg-pink-900 text-pink-800 dark:text-pink-200 rounded text-xs">{{ level }}</span>
                        </div>
                      </div>
                      <p v-else class="text-gray-500 text-sm">No calculus/stains noted</p>
                    </div>

                    <!-- Missing Teeth Summary -->
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow border border-gray-200 dark:border-gray-700">
                      <h5 class="font-semibold mb-3 flex items-center text-gray-800 dark:text-gray-200">
                        <X class="h-4 w-4 mr-2 text-gray-600"/>
                        Missing Teeth
                      </h5>
                      <div v-if="selectedExamination.missing_teeth && selectedExamination.missing_teeth.length > 0" class="flex flex-wrap gap-2">
                        <span v-for="tooth in selectedExamination.missing_teeth" :key="`missing-${tooth}`" class="bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 px-2 py-1 rounded-md text-xs font-medium">
                          #{{ tooth }}
                        </span>
                      </div>
                      <p v-else class="text-gray-500 text-sm">No missing teeth</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <!-- CREATE MODE -->
          <div v-else-if="mode === 'create'" class="h-full flex flex-col overflow-hidden">
            ...
            <div class="flex-grow overflow-hidden grid grid-cols-12 gap-0">
              ...
              <div class="col-span-12 lg:col-span-9 p-4 overflow-y-auto relative">
                ...
                <div class="flex flex-col items-center space-y-6">
                  ...
                  <div class="bg-white dark:bg-gray-800 rounded-xl p-3 shadow-lg border border-gray-200 dark:border-gray-700 w-full">
                      ...
                      <div class="grid grid-cols-16 gap-x-0.5 gap-y-1 justify-items-center relative max-w-7xl mx-auto">
                          ...
                          <div v-for="tooth in adultPermanent.upperRight" :key="`chart-${tooth}`" 
                              ...
                              <div class="mb-0.5 min-h-[16px]">
                                <div v-if="getToothCurrentCondition(tooth)" 
                                     class="text-xs font-medium px-0.5 py-0.5 rounded" 
                              ...
                              <!-- Condition label for create mode -->
                              <div v-if="mode === 'create' && newExaminationData.condition_of_teeth[tooth]" class="mt-0.5">
                      ...
                      <div class="grid grid-cols-10 gap-x-0.5 gap-y-1 justify-items-center relative max-w-5xl mx-auto">
                  ...
                  <div class="w-full">
                    <div class="space-y-4">
                      ...
                      <!-- Second Row: Mobility and Calculus Assessment -->
                      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <!-- Mobility Assessment -->
                        <div class="bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-900/20 dark:to-amber-800/20 rounded-xl p-6 shadow-lg">
                          <h4 class="font-semibold mb-4 flex items-center gap-3 text-amber-800 dark:text-amber-300 border-b-2 border-amber-200 dark:border-amber-700 pb-3">
                            <div class="flex items-center justify-center w-10 h-10 bg-amber-200 dark:bg-amber-800 rounded-full">
                              <Zap class="h-5 w-5 text-amber-600 dark:text-amber-400"/>
                            </div>
                            <span class="text-lg">Mobility Assessment</span>
                          </h4>
                          <div class="space-y-2 max-h-64 overflow-y-auto bg-amber-50/50 dark:bg-amber-900/10 rounded-lg p-4">
                            <div v-for="tooth in getCurrentTeethArray()" :key="`mob-${tooth}`" class="flex items-center justify-between text-sm p-2 rounded-lg hover:bg-amber-100 dark:hover:bg-amber-900/20 transition-colors bg-white/50 dark:bg-gray-800/50">
                              <span class="font-medium text-gray-700 dark:text-gray-300">Tooth #{{ tooth }}</span>
                              <select v-model="newExaminationData.mobility_scores[tooth]" class="text-xs bg-white dark:bg-gray-600 border-amber-300 dark:border-amber-500 rounded-lg py-1 px-2 focus:ring-2 focus:ring-amber-400 focus:border-amber-400">
                                <option :value="undefined">N/A</option>
                                <option v-for="grade in mobilityGrades" :key="grade" :value="grade">Grade {{ grade }}</option>
                              </select>
                            </div>
                          </div>
                        </div>

                        <!-- Calculus & Stains Assessment -->
                        <div class="bg-gradient-to-br from-rose-50 to-rose-100 dark:from-rose-900/20 dark:to-rose-800/20 rounded-xl p-6 shadow-lg">
                          <h4 class="font-semibold mb-4 flex items-center gap-3 text-rose-800 dark:text-rose-300 border-b-2 border-rose-200 dark:border-rose-700 pb-3">
                            <div class="flex items-center justify-center w-10 h-10 bg-rose-200 dark:bg-rose-800 rounded-full">
                              <Heart class="h-5 w-5 text-rose-600 dark:text-rose-400"/>
                            </div>
                            <span class="text-lg">Calculus & Stains</span>
                          </h4>
                          <div class="space-y-2 max-h-64 overflow-y-auto bg-rose-50/50 dark:bg-rose-900/10 rounded-lg p-4">
                            <div v-for="tooth in getCurrentTeethArray()" :key="`calc-${tooth}`" class="flex items-center justify-between text-sm p-2 rounded-lg hover:bg-rose-100 dark:hover:bg-rose-900/20 transition-colors bg-white/50 dark:bg-gray-800/50">
                              <span class="font-medium text-gray-700 dark:text-gray-300">Tooth #{{ tooth }}</span>
                              <select v-model="newExaminationData.calculus_stains[tooth]" class="text-xs bg-white dark:bg-gray-600 border-rose-300 dark:border-rose-500 rounded-lg py-1 px-2 focus:ring-2 focus:ring-rose-400 focus:border-rose-400">
                                <option :value="undefined">N/A</option>
                                <option v-for="level in calculusLevels" :key="level" :value="level">{{ level }}</option>
                              </select>
                            </div>
                          </div>
                        </div>
                      ...
                      <!-- Third Row: Custom Conditions and Clinical Notes -->
                      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <!-- Custom Conditions -->
