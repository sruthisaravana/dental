 <template>
  ...
  <div class="lg:col-span-4">
        ...
        <div v-for="appointment in sortedTimelineAppointments" :key="appointment.id"
          class="absolute bg-white border rounded-xl shadow-lg p-3 cursor-pointer transition-all hover:shadow-xl overflow-hidden"
                         :class="getAppointmentCardClass(appointment)"
...
const getAppointmentPosition = (appointment) => {
  ...
  const minHeight = 80 // Minimum readable height
  ...
  // Calculate lane positioning with better spacing
  const laneWidth = Math.floor(100 / totalLanes)
  const laneGap = totalLanes > 1 ? 1 : 0 // Small gap between lanes
  const actualWidth = laneWidth - laneGap
  const leftPosition = (laneWidth * layout.lane) + (laneGap / 2)
...
const fetchDailySchedule = async (date = null) => {
  ...
  timelineAppointments.value = appointments.filter(apt => apt.timeline_added && (apt.scheduled_at || apt.actual_start_at))
    ...
    const inProgressAppointment = allAptsToCheck.find(a => 
      (a.status === 'in-progress' || a.status === 'started') && 
      a.actual_start_at && 
      !a.actual_end_at
    )
    ...
    console.log('Checking for in-progress appointments:', {
      ...
      inProgressFound: !!inProgressAppointment,
    ...
    if (inProgressAppointment) {
      ...
      if (shouldResume) {
        ...
        timerMinutes.value = Math.floor(elapsedSeconds / 60)
        timerSeconds.value = elapsedSeconds % 60
        
        // Start the timer
        timing.value = true
        startTimer(timerMinutes.value, timerSeconds.value)
        
        console.log(`Auto-resumed timer for ${inProgressAppointment.patient?.first_name} ${inProgressAppointment.patient?.last_name}`)
        console.log(`Elapsed time: ${timerMinutes.value}:${timerSeconds.value.toString().padStart(2, '0')}`)
      }
      
    } else if (timing.value && !inProgressAppointment) {
      // No in-progress appointment found but timer is running - stop it
      console.log('Stopping timer - no in-progress appointment found')
      timing.value = false
      stopTimer()
      activeAppointment.value = null
    }
...
const addPatientToQueue = async () => {
  ...
  try {
    ...
    const appointmentData = {
      ...
      treatment: appointmentReason.value || 'General Checkup',
      reason: appointmentReason.value || 'General Checkup',
...
const startTimer = (resumeFromMinutes = 0, resumeFromSeconds = 0) => {
  ...
  // If resuming, start from provided values; otherwise start from 0
  if (resumeFromMinutes > 0 || resumeFromSeconds > 0) {
